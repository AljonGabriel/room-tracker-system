...
export const delSchedByProff = async (req, res) => {
  try {
    ...
    const professor = await Employee.findById(profID);
    if (!professor) {
      return res.status(404).json({ message: 'Professor not found' });
    }

    // Delete all rooms assigned to this professor
    const result = await Room.deleteMany({ professor: profID });

    res.status(200).json({
      message: `Successfully deleted all schedules/rooms assigned to ${professor.fullName}`,
      deletedCount: result.deletedCount,
    });
  } catch (error) {
    console.error('Error deleting schedules by professor:', error);
    res.status(500).json({
